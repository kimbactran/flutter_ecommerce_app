...
import 'package:ecommerce_app/data/services/firebase_storage_service.dart';
...
class CategoryRepository extends GetxController {
  ...
  Future<List<CategoryModel>> getAllCategories() async {
    try {
      ...
      throw 'Something went wrong when get all categories. Please try again!';
    }
  }
  ...
  Future<void> uploadDummyData(List<CategoryModel> categories) async {
    try {
      ...
      final storage = Get.put(EcoFirebaseStorageService());

      // Loop through each category
      for (var category in categories) {
        // get ImageData Link form the local assets
        final file = await storage.getImageDataFromAssets(category.image);

        // Upload Imgae and get its URL
        final url =
