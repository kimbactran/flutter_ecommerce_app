...
class CartController extends GetxController {
  ...
  void addToCart(ProductModel product) {
    ...
    if (productQuantityInCart.value < 1) {
      EcoLoader.customToast(message: 'Select Quantity');
      return;
    }

    if (product.productType == ProductType.variable.toString() &&
        variationController.selectedVariation.value.id.isEmpty) {
      EcoLoader.customToast(message: 'Select Variation');
      return;
    }
    ...
    updateCart();
    EcoLoader.customToast(message: "Your Product has been added to the Cart");
  }
  ...
  void removeFromCartDialog(int index) {
    Get.defaultDialog(
        ...
        onConfirm: () {
          ...
          cartItems.removeAt(index);
          updateCart();
          EcoLoader.customToast(message: 'Product removed from the Cart');
          Get.back();
        },
        onCancel: () => () => Get.back());
  }
  ...
  void updateAlreadyAddedProductCount(ProductModel product) {
    ...
    // Else make default entries to 0 and show cartEntries when variation selected.
  ...
  void saveCartItems() {
    ...
    EcoLocalStorage.instance().saveData('cartItems', cartItemStrings);
  }

  void loadCartItems() {
    final cartItemStrings =
        EcoLocalStorage.instance().readData<List<dynamic>>('cartItems');
